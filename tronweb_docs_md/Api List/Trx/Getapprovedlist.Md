

* [API List](/docu/docs/API List/)
* [trx](/docu/docs/API List/trx/)
* getApprovedList
Version: 6.3.0

On this page

# getApprovedList [<src>](https://github.com/tronprotocol/tronweb/blob/v6.3.0/src/lib/trx.ts#L782)

This function returns a JSON object with the list of accounts that have signed the transaction and the transaction data.

## Usage[​](#usage "Direct link to Usage")

```
tronWeb.trx.getApprovedList(transactionObj);
```

## Parameter[​](#parameter "Direct link to Parameter")

Object - A signed transaction.

## Return[​](#return "Direct link to Return")

Object - The list of accounts that have signed the transaction and the transaction data, see the example below.

## Example[​](#example "Direct link to Example")

```
await tronWeb.trx.getApprovedList({'visible': true, 'signature': ['1fd210045f5bdcf375cd478cf46ff735f132281b990bc199acf1952bd438929d1d03e12de5ea7dcb89cff5b8cfc5d161661a5c1fe6a6a2422edb313b9139075300'], 'txID': 'ee188aaf5cf78729d2d14d4db698126da2d75ef78a43837dafd6e6f591d103a2', 'raw_data': {'contract': [{'parameter': {'value': {'amount': 125000000, 'owner_address': 'TN9RRaXkCFtTXRso2GdTZxSxxwufzxLQPP', 'to_address': 'TTSFjEG3Lu9WkHdp4JrWYhbGP6K1REqnGQ'}, 'type_url': 'type.googleapis.com/protocol.TransferContract'}, 'type': 'TransferContract'}], 'ref_block_bytes': 'c251', 'ref_block_hash': '5c685c92bf035e72', 'expiration': 1578299967000, 'timestamp': 1578299909600}, 'raw_data_hex': ''});  
{  
  result: {},  
  approved_list: [ 'TN9RRaXkCFtTXRso2GdTZxSxxwufzxLQPP' ],  
  transaction: {  
    result: { result: true },  
    txid: 'ee188aaf5cf78729d2d14d4db698126da2d75ef78a43837dafd6e6f591d103a2',  
    transaction: {  
      signature: [Array],  
      txID: 'ee188aaf5cf78729d2d14d4db698126da2d75ef78a43837dafd6e6f591d103a2',  
      raw_data: [Object],  
      raw_data_hex: ''  
    }  
  }  
}
```